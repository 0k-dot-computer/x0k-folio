...
x0k:
  ...
  type: implementation
  status: draft
  summary: The filesystem half the pure region weaver leaves out — resolving a publication's members with nothing but the envelope parser and a small table of corpus layout, shared by the CLI and the MCP tool so the two cannot drift.
  concerns: [tangle, publication, projection, io]
  tangle:
    crate: x0k-tangle
...
from `x0k-folio` and a small table of where each document class lives in
the corpus, rather than depending on the corpus's own graph service. That
table is the one piece of corpus layout this crate carries, and it is the
invariant every other reader of the corpus must agree with: a member URI
...
under `decisions/<subtype>/…` or `knowledge/wiki/…`, weaves them, wraps the
