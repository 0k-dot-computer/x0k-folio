...
to this one: `check` reads any directory of Markdown. Read the README's "Your
first typed document" for the envelope and what each line means, then write
the envelope by hand — no verb writes it for you:

- `format: folio/v1`, an `id` with the `x0k:` scheme (the parser admits no
  other), a `type` (`commitment`, `architecture`, or `design` for a decision;
  `implementation` for a document that tangles code; `wiki`, `manuscript`,
  `publication`), a `status` (`proposed`, `accepted`, `superseded`), and
  `edges` whose predicates the `document` module declares —
  `x0k-ontology/ontology/modules/document.ttl`, one `rdfs:comment` per
  predicate saying what it means and `rdfs:domain`/`rdfs:range` saying what
  it may connect.
