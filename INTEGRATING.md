...
`cargo run --locked -q -p x0k-tangle --bin x0k-tangle -- tangle corpora/x0k/implementation --workspace .`.
The projector emits that path from this publication's source layout.
...
- **A configurable root for the legacy `workspace` verb.** Its identity
  ...
  works anywhere under the workspace; this repository's CI uses that path.
