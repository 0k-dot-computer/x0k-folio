...
This module defines the *contract* between tangle — the
[literate](../../background/literate-programming.md "x0k:wiki/literate-programming") chunk-to-source step — and its
plugins —
...
<a name="chunk-module-header"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#module-header`</sub>
...
<a name="chunk-imports"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#imports`</sub>
...
<a name="chunk-trait-def"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#trait-def`</sub>
...
<a name="chunk-pipeline-context"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#pipeline-context`</sub>
...
<a name="chunk-chunk-input"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#chunk-input`</sub>
...
The header is not decoration. `tools/generated-drift`,
`tools/fmt-churn-guard.py`, the receive path's refusal, and the
`x0k-guard-generated` script the repository projector writes all
recognize a generated file by grepping its first line for
`@generated`. So a wrong marker is a file that will not parse, and no
marker is a file nothing protects — which is why `None` is a
deliberate declaration (JSON, binaries) rather than a fallback.

<a name="chunk-comment-style"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#comment-style`</sub>
...
The `&'static str` slots keep the type `Copy` and keep a plugin from
constructing a marker out of arbitrary runtime data by accident. A
plugin that legitimately needs a marker from *outside* the program —
`IdentityPipeline` reads one from a document's `comment_styles:`
config — interns it into a `'static` slot on its own side of the
boundary, so the cost of the openness lands there rather than on every
construction site in the substrate.
...
<a name="chunk-pipeline-output"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#pipeline-output`</sub>
...
`tangle` overwrites its outputs. That is the whole job — the document
is canonical and the projection follows it. The failure it made
possible is the one a contributor reported: they added a line to a
generated file, re-tangled, and the line was gone. No message, exit 0.
The sidecar had recorded a hash for that path, so the tool could see
the bytes on disk were not the bytes it wrote; it simply never looked.

This section is the looking. It is *pure* — three content hashes in, a
verdict out — because the protocol module holds no I/O and the
dispatcher already computes exactly these hashes on its way to
`write_atomic`.

The three inputs are the design. `recorded` is what the sidecar says
this document last wrote to the path; `on_disk` is what the file reads
now; `about_to_write` is what this run composed. The question is
*disk against what we last wrote*, never *disk against what we are
about to write* — a document that changed is supposed to change its
output, and a check phrased the second way would refuse every real
edit in the corpus.

<a name="chunk-output-provenance"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#output-provenance`</sub>

```rust {#output-provenance}
...
The arm order carries two decisions worth naming. `AlreadyCurrent` sits
above `Unrecorded` and `Foreign` because a write that changes no byte
cannot destroy one, whatever the sidecar remembers — that is what keeps
a stale sidecar from turning a no-op re-tangle into a refusal, and it is
why `tools/ci`'s re-tangle-and-diff still runs on a tree whose sidecars
have drifted. And `Unrecorded` is a category rather than a synonym for
`Foreign`: refusing there would block converting a hand-written file to
literate authoring, which is a workflow this substrate asks people to
perform.

## Refuse, not warn

Only `Foreign` is a destruction. What the run does about it is a
policy, because the operator sometimes means it.

<a name="chunk-clobber-policy"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#clobber-policy`</sub>
...
Refusing is the stronger choice, and the corpus says so three ways.

*It costs almost nothing.* Audited across this workspace, 1128 of 1130
recorded outputs hash exactly to the file on disk; the two exceptions
were outputs another lane was mid-edit on. A refusal that fires twice
in eleven hundred is not a tax on the common path — it is a tripwire
that has never been touched by ordinary work.

*The costs are asymmetric.* The reported case is an edit that was never
committed, so destroying it destroys it for good. Refusing costs one
flag. When one arm of a choice is unrecoverable and the other is a
retry, the tool takes the retry.

*A warning here would not be read.* `tangle workspace` walks 836
documents; the complaint that opened this section was precisely that
the tool said nothing, and a line scrolling past in a run that long is
not far from nothing. The refusal stops the run and names the file.

There is a counter-precedent in this same crate, and it is worth being
straight about it: [`identity-pipeline.md`](identity-pipeline.md)
argues *against* refusing when a comment style is unknown, on the
grounds that a tangler which can decline is a large behavioural promise
to break. The distinction is what the tool knows. There it declines out
of ignorance — it has no idea whether `//` is wrong, and usually it
isn't. Here it declines on a measurement: it recorded a hash, it read
the file, and the two disagree. Declining on evidence is a different
act from declining on a guess.

`Unrecorded` stays a warning for the reason its doc-comment gives: the
tool genuinely cannot tell a lost sidecar from a file being adopted
into a document, and refusing would break the adoption.

## Screening a document's outputs

One call per document, not per file. The dispatcher classifies every
output first and screens them together, so a document with three
outputs and one clobber writes none of them instead of two — an
all-or-nothing that matters because a half-written module compiles
about as often as it doesn't.

<a name="chunk-guard-outputs"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#guard-outputs`</sub>
...
The dispatcher owns the three hashes this reads. It already computes
two of them — `content_hash_bytes` over the composed bytes on its way
to the sidecar record — and the third is a `std::fs::read` of the path
it is about to write, against the sidecar it can load beside the
document. What it must add is the shape: compose every output of a
pipeline into a vec, call `guard_outputs` once, and only then write.
...
<a name="chunk-error-types"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#error-types`</sub>
...
<a name="chunk-registry"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#registry`</sub>
...
The clobber-guard tests are the other half, and they are written as one
test per failure mode rather than one per verdict, because the verdicts
are cheap and the failure modes are what the guard was bought with: a
first tangle with no sidecar entry, a document that changed so its
output must, a hand edit that has to be caught, a write that changes
nothing and therefore destroys nothing, an unclaimed file being adopted,
a deleted output regenerating, `--force`, and the all-or-nothing refusal
across a document's outputs.

<a name="chunk-tests"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#tests`</sub>
...
<a name="chunk-root"></a><sub>[`src/pipeline.rs`](../../crates/x0k-tangle/src/pipeline.rs) · `#root` · assembles [module-header](#chunk-module-header) · [imports](#chunk-imports) · [trait-def](#chunk-trait-def) · [pipeline-context](#chunk-pipeline-context) · [chunk-input](#chunk-chunk-input) · [comment-style](#chunk-comment-style) · [pipeline-output](#chunk-pipeline-output) · [output-provenance](#chunk-output-provenance) · [clobber-policy](#chunk-clobber-policy) · [guard-outputs](#chunk-guard-outputs) · [error-types](#chunk-error-types) · [registry](#chunk-registry) · [tests](#chunk-tests)</sub>
