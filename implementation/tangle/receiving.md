...
— the published crates, the corpus's implementation root, the
publication docs, the class registry that names both, and the
vocabulary — at any commit in well under a second, with no checkout. When that works, the reference is exact. When it cannot
...
<a name="chunk-published-crates"></a><sub>[`src/receive.rs`](../../crates/x0k-tangle/src/receive.rs) · `#published-crates`</sub>
