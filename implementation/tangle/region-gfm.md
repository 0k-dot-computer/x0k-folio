...
<a name="chunk-module-doc"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#module-doc`</sub>
...
<a name="chunk-imports"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#imports`</sub>
...
<a name="chunk-segments"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#segments`</sub>
...
<a name="chunk-chapter-links"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#chapter-links`</sub>
...
<a name="chunk-weave-chapter"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#weave-chapter`</sub>
...
/// Add the same reversible instance captions to a named prose document.
pub fn weave_instance_declarations(
    text: &str, source: &str, instances: &crate::instance_rendering::InstancePresentation,
) -> Result<String> {
    let mut out = String::new();
    let mut instance_occurrence = 0;
    for segment in segments(text) {
        match segment {
            Segment::Line(line) => {
                if line.starts_with(crate::instance_rendering::INSTANCE_CAPTION_MARK) {
                    bail!("source already carries a woven instance caption");
                }
                out.push_str(line);
            }
            Segment::Fence { open, info, body, close } => {
                if let Some(mut view) = instances.view(source, &info, &body.concat()) {
                    instance_occurrence += 1;
                    view.occurrence = instance_occurrence;
                    out.push_str(&crate::instance_rendering::gfm(&view));
                }
                out.push_str(open);
                for line in body { out.push_str(line); }
                if let Some(close) = close { out.push_str(close); }
            }
        }
    }
    Ok(out)
...
pub fn weave_chapter(
    ...
    let instances = crate::instance_rendering::InstancePresentation::same_document(rel, text);
    weave_chapter_with_instances(text, rel, crate_name, links, &instances)
}

pub fn weave_chapter_with_instances(
    ...
    for segment in segments(body) {
        match segment {
            ...
            Segment::Fence { open, info, body, close } => {
                ...
                if let Some(mut view) = instances.view(rel, &info, &body.concat()) {
                    instance_occurrence += 1;
                    view.occurrence = instance_occurrence;
                    out.push_str(&crate::instance_rendering::gfm(&view));
                }
...
fn refuse_woven_line(line: &str, rel: &str) -> Result<()> {
    if line.starts_with(CAPTION_MARK) || line.starts_with(crate::instance_rendering::INSTANCE_CAPTION_MARK) || rewritten_link(line).is_some() {
        bail!(
...
<a name="chunk-rewrite-links"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#rewrite-links`</sub>
...
pub fn unweave_chapter(text: &str) -> String {
    ...
    while let Some(line) = lines.next() {
        if line.starts_with(CAPTION_MARK) || line.starts_with(crate::instance_rendering::INSTANCE_CAPTION_MARK) {
...
<a name="chunk-affordance-evidence"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#affordance-evidence`</sub>
...
fn render_evidence(ev: &AffordanceEvidence) -> String {
    ...
    if !ev.proofs.is_empty() {
        ...
        for p in &ev.proofs {
            // A test can itself contain Markdown fences in a string literal.
            // Its enclosing proof fence must be longer than every such run.
            let width = p.source.split(|c| c != char::from(96)).map(str::len).max().unwrap_or(0).max(2) + 1;
            let fence = char::from(96).to_string().repeat(width);
...
<a name="chunk-relative-link"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#relative-link`</sub>
...
<a name="chunk-tests"></a><sub>[`src/region_gfm.rs`](../../crates/x0k-tangle/src/region_gfm.rs) · `#tests` · proves [Read a document as the woven artifact](../../decisions/design/corpus/literate-programming/read-a-document-as-the-woven-artifact.md)</sub>
