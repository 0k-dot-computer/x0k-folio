...
<a name="chunk-module-header"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#module-header`</sub>
...
//!
//! See `corpora/x0k/decisions/design/corpus/literate-pipelines.md` for the full spec.
```

## Imports

<a name="chunk-imports"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#imports`</sub>
...
<a name="chunk-sidecar-types"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#sidecar-types`</sub>
...
<a name="chunk-run-result-types"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#run-result-types`</sub>
...
<a name="chunk-sidecar-path"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#sidecar-path`</sub>
...
<a name="chunk-tangle-lock-state"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-lock-state`</sub>
...
<a name="chunk-tangle-lock-guard"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-lock-guard`</sub>
...
<a name="chunk-tangle-lock-acquire"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-lock-acquire`</sub>
...
<a name="chunk-tangle-lock-path"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-lock-path`</sub>
...
<a name="chunk-containment-fns"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#containment-fns`</sub>
...
## Where the documents can be read

The header the next section prepends names its source document by
workspace-relative path — `corpora/x0k/implementation/tangle/dispatcher.md`.
That is a live reference for a reader holding the tree the tangle ran in,
and a dead one for everybody else. Our own shop window says so: the
published `x0k-folio` repository opens its `README.md` with

    <!-- @generated by x0k-tangle (pipeline: identity-tangle) from corpora/x0k/publications/x0k-folio/x0k-folio.md — DO NOT EDIT. -->

and `corpora/x0k/publications/` is not in that repository. An adopter met
the same wall from the other side: their `package.json` ships `src` and not
`lit`, so every consumer who opens the npm tarball's source reads
`from lit/horizon-remap.md — DO NOT EDIT` naming a file the tarball does not
carry. Their choice was a dangling breadcrumb or a doubled tarball.

A `DocSite` is the missing half: a base URL the header's path resolves
under, so the line can say both where the document is in the tree it was
tangled from *and* where a stranger can read it.
...
document. Putting it in parentheses after the path leaves the first token
after ` from ` exactly what it was, and gives the guard's message a
clickable tail for free.

It is **inert when no site is known**. `DocSite` is an `Option` that
defaults to `None`, and `None` composes today's line byte for byte. A
monorepo tangle — where the path in the header *is* where the document
lives, for everyone who can read the code at all — adds nothing, so no
generated file in this tree changes.
...
site says where *the documents of this run* can be read. It cannot rescue
the `README.md` header above, because the publication document that
generates it stays in the private corpus — there is no URL for it to name.
That header's dangling path is a projector-side question (carry the
document, or say "corpus-private" instead of naming a path), not one a
base URL can answer.

<a name="chunk-doc-site"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#doc-site`</sub>
...
<a name="chunk-tangle-document"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-document`</sub>
...
literate tree through, so it takes settings too: every document under one
directory of one repository shares a document site, which is exactly the
grain [`DocSite`](#where-the-documents-can-be-read) is cut at. The
workspace pass ([`tangle_workspace`](#workspace-tangling)) deliberately
has no such variant — it sweeps the roots the *registry's* plugins claim
in the tree the tangler is running in, and a run that knows a public URL
for its documents is publishing a known set of them, not sweeping.
...
<a name="chunk-freshness-types"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#freshness-types`</sub>
...
<a name="chunk-doc-freshness-fn"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#doc-freshness-fn`</sub>
...
<a name="chunk-tangle-workspace-fn"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tangle-workspace-fn`</sub>
...
<a name="chunk-collision-error-fn"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#collision-error-fn`</sub>
...
<a name="chunk-compose-with-header-fn"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#compose-with-header-fn`</sub>
...
<a name="chunk-temp-sibling-fn"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#temp-sibling-fn`</sub>
...
<a name="chunk-hash-fns"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#hash-fns`</sub>
...
pub(crate) fn content_hash_bytes(b: &[u8]) -> String {
    let mut h = DefaultHasher::new();
    b.hash(&mut h);
    format!("{:016x}", h.finish())
}
...
<a name="chunk-tests-machinery"></a><sub>[`src/pipeline_runner.rs`](../../crates/x0k-tangle/src/pipeline_runner.rs) · `#tests-machinery`</sub>
