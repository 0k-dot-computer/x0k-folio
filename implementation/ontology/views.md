...
<a name="chunk-module-doc"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#module-doc`</sub>
...
The hand-written `pub mod`s below are the other kind, and they are a
shrinking set — two now: what is left of the chapters that spelled a
vocabulary the concept region did not hold, entries and settlements.
Their *terms* have been leaving for modules one chapter at a time
(`x0k:architecture/ontology-modules` §7); what stays behind is the part no
`.ttl` can declare — a minting function, a closed enumeration as a Rust
type, an instance-URI prefix. They are about x0k's own workings and no
consumer of the fold calls them, so they sit behind a `product` feature
that is off by default: `concept_facts` and the generated views are what a
plain build gets, and a workspace that wants one of these asks for the
feature on its own dependency line.

The set shrinks from the other end too, and that is the exit. A residue
with exactly one consumer crate is not a vocabulary crate's business: the
declaration shapes went to `x0k-entry-store`, beside the one function that
consumes a `VocabularyConcept`, and the desired-event shape went with them,
beside the conformance test that is its only caller. The proposal and
finding residues went to `x0k-triage` the same way: their readers' callers
are the cell's own modules, and `servitor-rust-host` reaches them over the
dependency edge it already had on the cell. What stays is the residue
several crates read — `entry_shape` and `settlement_fold`, each read by
crates that are siblings with no shared lower crate, which is a placement
question and not a move.

That is a feature gate rather than a second crate because nothing here is
being built — the residue already exists and its call sites already import
it. The gate says which of them a build knows; splitting the crate would say
it with a version seam instead, and there is no adopter for whom that is the
better answer. The feature's exit condition is not "the last chapter leaves"
but "the last residue finds a home".

<a name="chunk-modules"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#modules`</sub>
...
<a name="chunk-shipped"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#shipped`</sub>
...
<a name="chunk-lookups"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#lookups`</sub>
...
<a name="chunk-tests"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#tests`</sub>
...
<a name="chunk-root"></a><sub>[`src/lib.rs`](../../crates/x0k-ontology/src/lib.rs) · `#root` · assembles [module-doc](#chunk-module-doc) · [modules](#chunk-modules) · [shipped](#chunk-shipped) · [lookups](#chunk-lookups) · [tests](#chunk-tests)</sub>
